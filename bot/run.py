...
bot = telebot.TeleBot('2057789951:AAHOJqUXOdFW72Mn1FWPxgH850R8QLl5CXk')
...
def listar_sinais(session):
    bot.send_message(session.chat.id, str(
        '##### Listando Sinais ('+str(len(lista_sinais))+') #####'))
...
def remover_sinal(session):
    ...
    for sinal in lista_sinais:
        if int(sinal[0]) == int(position):
            ...
            bot.reply_to(session, str('Sinal da posição : ' +
                         position+' removido com sucesso. '))
        ...
        for sinal in lista_sinais:
            ...
            if str(sinal[1]) == minutos:
                status, id = API.buy_digital_spot(
                    str(sinal[2]), int(sinal[3]), str(sinal[4]), int(sinal[5]))
                if status:
                    while True:
                        resultado, valor = API.check_win_digital_v2(id)
                        if resultado:
                            if valor > 0:
                                bot.send_message(session.chat.id, str('###### Resultado: Win ######\nDados Op:'+sinal[2]+' | '+str(
                                    sinal[3])+',00 | '+sinal[4]+' | '+str(sinal[5])+' min\nLucro: '+str(round(valor, 2))+'\n####################'))
                                lista_sinais.remove(sinal)
                                bot.send_message(session.chat.id, str(
                                    'Sinal da posição : '+str(sinal[0])+' removido. '))
                                break
                            elif valor < 0:
